...
def plot_length_distributions(df, output_prefix):
    ...
    bin_size = 5000
    max_bin = 100000  # anything >= 1Mb goes in final bin
    ...
    for label, cond in conditions:
        ...
        print(f">=100,000 | {overflow_count}")

        # Plot histogram
        plt.figure(figsize=(10, 5))
        # create bin labels for plotting
        bin_labels = [f"{int(bin_edges[i])}-{int(bin_edges[i+1]-1)}" for i in range(len(counts)-1)] + [">100Kb"]
