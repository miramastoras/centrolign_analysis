...
def plot_length_distributions(df, output_prefix):
    ...
    for label, cond in conditions:
        ...
        # Use bin edges (excluding last edge) as bar positions for start-aligned labels
        x_pos = bin_edges[:-1]

        # Plot bars with width = bin_size, align='edge' ensures they start at x_pos
        plt.bar(x_pos, counts, width=bin_size, align='edge', color="skyblue", edgecolor="black", alpha=0.7)

        # x-axis labels: start of each bin, final bin labeled '>1Mb'
        x_labels = [str(int(edge)) for edge in bin_edges[:-1]] + [">50kb"]
