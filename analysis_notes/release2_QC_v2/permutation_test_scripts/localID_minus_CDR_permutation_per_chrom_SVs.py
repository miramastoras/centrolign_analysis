...
region (top 10% minus CDR) have an elevated SV rate compared to the rest of
...
Usage:
    python localID_minus_CDR_permutation_per_chrom_SVs.py --n-reps 10000 --n-workers 8 --output-dir /path/to/output
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome localID-minus-CDR permutation test for SVs (parallelized)"
    )
    ...
    null_out = os.path.join(args.output_dir, "localid_minus_cdr_sv_null_distribution_per_chr.csv")
    ...
    results_out = os.path.join(args.output_dir, "localid_minus_cdr_sv_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (top10% local ID minus CDR, SVs):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
