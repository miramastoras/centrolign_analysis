#!/usr/bin/env python3
"""
Per-chromosome permutation test: do the top 10% local identity bins
have an elevated SNV mutation rate compared to the rest of the array?

Parallelizes across chromosomes using multiprocessing.

Usage:
    python local_identity_permutation_per_chrom.py --n-reps 1000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, snv_parquet, asat_bed_dir, local_id_bed_dir, cigar_csv_dir):
    ...
    print("Loading SNVs...")
    SNVs_raw_df = pd.read_parquet(snv_parquet)
    ...
    # Top 10% local identity bins per sample/chr
    thresholds = (
    ...
    snv_pos = defaultdict(list)
    for row in SNVs_raw_df.itertuples(index=False):
        snv_pos[(row.sample1, row.chr, row.sample_pair)].append(row.ref_start)
        snv_pos[(row.sample2, row.chr, row.sample_pair)].append(row.query_start)
    ...
    return pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals
...
def _init_worker(pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals, n_reps):
    global _pairs_df, _real_local_id_intervals, _asat_bounds, _snv_pos, _aligned_intervals, _n_reps
    _pairs_df = pairs_df
...
def _run_chromosome(chr_):
    ...
    for rep in range(_n_reps):
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            if (
                (s1, chr_) not in _real_local_id_intervals
                or (s2, chr_) not in _real_local_id_intervals
                or len(_real_local_id_intervals[(s1, chr_)]) == 0
                or len(_real_local_id_intervals[(s2, chr_)]) == 0
            ):
                continue
...
def compute_observed_test_stats(pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals):
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome local identity permutation test (parallelized)"
    )
    ...
    snv_parquet = "/private/groups/patenlab/mira/centrolign/analysis/CDR_permutation/SNVs_raw_df.parquet"
    ...
    pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals = load_all_data(
        pairwise_dist_dir, snv_parquet, asat_bed_dir, local_id_bed_dir, cigar_csv_dir
    )
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "local_id_null_distribution_per_chr.csv")
    ...
    per_chrom_results_df = compute_observed_test_stats(
        pairs_df, real_local_id_intervals, asat_bounds, snv_pos, aligned_intervals
    )
    ...
    results_out = os.path.join(args.output_dir, "local_id_per_chrom_results.csv")
