...
region (top 10% minus CDR) have an elevated SNV rate compared to the rest
of the array (outside both CDR and top 10%)?

Just restricting to samples with <0.05 distance 
...
Usage:
    python localID_minus_CDR_permutation_per_chrom_SNVs.py --n-reps 10000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, snv_parquet, asat_bed_dir, cdr_idx_csv,
    ...
    all_pairs_dist_df = all_pairs_dist_df[all_pairs_dist_df["direct_pairwise_dist"] < 0.05]
    ...
    for rep in range(_n_reps):
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                # test_region = fake_top10 - fake_CDR (INVERTED from CDR_minus_localID)
    ...
    for chrom in pairs_df['chr'].unique():
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                # test_region = real top 10% - real CDR (INVERTED from CDR_minus_localID)
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome localID-minus-CDR permutation test for SNVs (parallelized)"
    )
    ...
    null_out = os.path.join(args.output_dir, "localid_minus_cdr_snv_null_distribution_per_chr.csv")
    ...
    results_out = os.path.join(args.output_dir, "localid_minus_cdr_snv_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (top10% local ID minus CDR, SNVs):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
