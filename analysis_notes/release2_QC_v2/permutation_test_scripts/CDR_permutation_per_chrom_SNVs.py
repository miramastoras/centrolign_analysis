...
Usage:
    python CDR_permutation_per_chrom_SNVs.py --n-reps 1000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, snv_parquet, asat_bed_dir, cdr_idx_csv, cigar_csv_dir):
    ...
    return pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals
...
def _init_worker(pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals, n_reps):
    global _pairs_df, _real_cdr_intervals, _asat_bounds, _snv_pos, _aligned_intervals, _n_reps
    _pairs_df = pairs_df
...
def compute_observed_test_stats(pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals):
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome CDR permutation test for SNVs (parallelized)"
    )
    ...
    pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals = load_all_data(
        pairwise_dist_dir, snv_parquet, asat_bed_dir, cdr_idx_csv, cigar_csv_dir
    )
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "cdr_null_distribution_per_chr.csv")
    ...
    per_chrom_results_df = compute_observed_test_stats(
        pairs_df, real_cdr_intervals, asat_bounds, snv_pos, aligned_intervals
    )
    ...
    results_out = os.path.join(args.output_dir, "cdr_per_chrom_results.csv")
