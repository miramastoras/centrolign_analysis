...
Usage:
    python local_identity_permutation_per_chrom_svs.py --n-reps 1000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, sv_parquet, asat_bed_dir, local_id_bed_dir):
    ...
    return pairs_df, real_local_id_intervals, asat_bounds, sv_pos
...
def _init_worker(pairs_df, real_local_id_intervals, asat_bounds, sv_pos, n_reps):
    global _pairs_df, _real_local_id_intervals, _asat_bounds, _sv_pos, _n_reps
    _pairs_df = pairs_df
...
def compute_observed_test_stats(pairs_df, real_local_id_intervals, asat_bounds, sv_pos):
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome local identity permutation test for SVs (parallelized)"
    )
    ...
    pairs_df, real_local_id_intervals, asat_bounds, sv_pos = load_all_data(
        pairwise_dist_dir, sv_parquet, asat_bed_dir, local_id_bed_dir
    )
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_local_id_intervals, asat_bounds, sv_pos, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "local_id_sv_null_distribution_per_chr.csv")
    ...
    per_chrom_results_df = compute_observed_test_stats(
        pairs_df, real_local_id_intervals, asat_bounds, sv_pos
    )
    ...
    results_out = os.path.join(args.output_dir, "local_id_sv_per_chrom_results.csv")
