#!/usr/bin/env python3
"""
Per-chromosome permutation test: does the CDR-exclusive region (CDR minus
top 10% local identity) have an elevated SNV rate compared to the rest of
the array (outside both CDR and top 10%)?

test_region      = CDR - top_10%_local_identity
comparison_region = complement(CDR ∪ top_10%_local_identity)

Parallelizes across chromosomes using multiprocessing.

Usage:
    python CDR_minus_localID_permutation_per_chrom_SNVs.py --n-reps 10000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, snv_parquet, asat_bed_dir, cdr_idx_csv,
    ...
    return (pairs_df, real_cdr_intervals, real_local_id_intervals,
            asat_bounds, snv_pos, aligned_intervals)
...
def _init_worker(pairs_df, real_cdr_intervals, real_local_id_intervals,
                 asat_bounds, snv_pos, aligned_intervals, n_reps):
    global _pairs_df, _real_cdr_intervals, _real_local_id_intervals
    global _asat_bounds, _snv_pos, _aligned_intervals, _n_reps
    _pairs_df = pairs_df
    ...
    for rep in range(_n_reps):
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                # test_region = fake_CDR - fake_top10
                test_region = interval_subtract(fake_cdr, fake_top10)
                ...
                rate_in = snv_rate(pos, aln, test_region)
                rate_out = snv_rate(pos, aln, comparison_region)
    ...
    for chrom in pairs_df['chr'].unique():
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                # test_region = real CDR - real top 10%
                test_region = interval_subtract(cdrs, top10)
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome CDR-minus-localID permutation test for SNVs (parallelized)"
    )
    ...
    (pairs_df, real_cdr_intervals, real_local_id_intervals,
     asat_bounds, snv_pos, aligned_intervals) = load_all_data(
        pairwise_dist_dir, snv_parquet, asat_bed_dir, cdr_idx_csv,
        local_id_bed_dir, cigar_csv_dir
    )
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_cdr_intervals, real_local_id_intervals,
                  asat_bounds, snv_pos, aligned_intervals, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "cdr_minus_localid_snv_null_distribution_per_chr.csv")
    ...
    results_out = os.path.join(args.output_dir, "cdr_minus_localid_snv_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (CDR minus top10% local ID, SNVs):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
