...
top 10% local identity) have an elevated short indel rate compared to the rest
...
Usage:
    python CDR_minus_localID_permutation_per_chrom_short_indels.py --n-reps 10000 --n-workers 8 --output-dir /path/to/output
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome CDR-minus-localID permutation test for short indels (parallelized)"
    )
    ...
    null_out = os.path.join(args.output_dir, "cdr_minus_localid_indel_null_distribution_per_chr.csv")
    ...
    results_out = os.path.join(args.output_dir, "cdr_minus_localid_indel_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (CDR minus top10% local ID, short indels):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
