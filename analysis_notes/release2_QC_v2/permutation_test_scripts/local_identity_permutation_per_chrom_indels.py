...
have an elevated short indel rate compared to the rest of the array?

Parallelizes across chromosomes using multiprocessing.

Usage:
    python local_identity_permutation_per_chrom_indels.py --n-reps 1000 --n-workers 8 --output-dir /path/to/output
"""
...
def load_all_data(pairwise_dist_dir, indel_parquet, asat_bed_dir, local_id_bed_dir, cigar_csv_dir):
    ...
    return pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals
...
def _init_worker(pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals, n_reps):
    global _pairs_df, _real_local_id_intervals, _asat_bounds, _indel_pos, _aligned_intervals, _n_reps
    _pairs_df = pairs_df
    ...
    for rep in range(_n_reps):
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                rate_in = variant_rate(pos, aln, fake_cdr)
                rate_out = variant_rate(pos, aln, fake_outside)
...
def compute_observed_test_stats(pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals):
    ...
    for chrom in pairs_df['chr'].unique():
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                rate_in = variant_rate(pos, aln, cdrs)
                rate_out = variant_rate(pos, aln, outside)
                if np.isfinite(rate_in) and np.isfinite(rate_out):
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome local identity permutation test for short indels (parallelized)"
    )
    ...
    pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals = load_all_data(
        pairwise_dist_dir, indel_parquet, asat_bed_dir, local_id_bed_dir, cigar_csv_dir
    )
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "local_id_indel_null_distribution_per_chr.csv")
    ...
    per_chrom_results_df = compute_observed_test_stats(
        pairs_df, real_local_id_intervals, asat_bounds, indel_pos, aligned_intervals
    )
    ...
    results_out = os.path.join(args.output_dir, "local_id_indel_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (short indels):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
