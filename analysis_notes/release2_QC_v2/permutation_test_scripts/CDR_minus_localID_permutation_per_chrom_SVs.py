...
top 10% local identity) have an elevated SV rate compared to the rest of
...
Usage:
    python CDR_minus_localID_permutation_per_chrom_SVs.py --n-reps 10000 --n-workers 8 --output-dir /path/to/output
"""
    ...
    for rep in range(_n_reps):
        ...
        for r in chr_pairs.itertuples(index=False):
            ...
            for sample in (s1, s2):
                ...
                rate_in = variant_rate(pos, test_region)
                rate_out = variant_rate(pos, comparison_region)
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-chromosome CDR-minus-localID permutation test for SVs (parallelized)"
    )
    ...
    (pairs_df, real_cdr_intervals, real_local_id_intervals,
     asat_bounds, sv_pos) = load_all_data(
    ...
    with Pool(
        ...
        initargs=(pairs_df, real_cdr_intervals, real_local_id_intervals,
                  asat_bounds, sv_pos, args.n_reps),
    ) as pool:
    ...
    null_out = os.path.join(args.output_dir, "cdr_minus_localid_sv_null_distribution_per_chr.csv")
    ...
    results_out = os.path.join(args.output_dir, "cdr_minus_localid_sv_per_chrom_results.csv")
    per_chrom_results_df.to_csv(results_out, index=False)
    print(f"Per-chrom results saved to {results_out}")

    # Print summary
    print("\n" + "=" * 80)
    print("Results summary (CDR minus top10% local ID, SVs):")
    print("=" * 80)
    for _, row in per_chrom_results_df.iterrows():
        p = row['p_value']
