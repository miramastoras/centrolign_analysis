...
SAMPLE1=$(awk "NR==$SLURM_ARRAY_TASK_ID" "$COMBINATIONS_FILE" | cut -f1 | xargs basename | cut -f1-2 -d"_")
SAMPLE2=$(awk "NR==$SLURM_ARRAY_TASK_ID" "$COMBINATIONS_FILE" | cut -f2 | xargs basename | cut -f1-2 -d"_")
