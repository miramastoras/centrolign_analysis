...
CHROMDIR=/private/groups/patenlab/mira/centrolign/batch_submissions/centrolign/release2_QC_v2/all_pairs/${CHR}_072326
...
# create config file for the run 
echo $TEMP_FASTA
sed "s|DUMMY_FASTA_NAME|$TEMP_FASTA|g" $CHROMDIR/chrY_config.yaml > ${WORKDIR}/${SAMPLE1}_${SAMPLE2}.config.yaml

time /private/home/mmastora/progs/centrolign/build/centrolign \
    -v 3 --skip-calibration \
    --config ${WORKDIR}/${SAMPLE1}_${SAMPLE2}.config.yaml \
    $TEMP_FASTA > $OUTDIR/pairwise_cigar_${SAMPLE1}_${SAMPLE2}.txt
